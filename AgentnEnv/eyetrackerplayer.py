...
class TMTReplayApp:
    ...
    def _load_asc(self):
        """Parses an EyeLink .asc file generated by edf2asc handling blink/missing values."""
        path = filedialog.askopenfilename(filetypes=[("EyeLink ASC files", "*.asc"), ("Text files", "*.txt")])
        if not path:
            return

        self.gaze_samples = []
        sample_pattern = re.compile(r"^(\d+)\s+(\S+)\s+(\S+)")
            ...
            for line in f:
                ...
                if match:
                    ...
                    if x_str == "." or y_str == ".":
                        continue
                    try:
                        self.gaze_samples.append((int(ts), float(x_str), float(y_str)))
                    except ValueError:
                        continue
