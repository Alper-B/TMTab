...
class TMTReplayApp:
    def __init__(self, root):
        self.root = root
        self.root.title("TMT Replay from Log")
    ...
    def _build_ui(self):
        ...
        ttk.Label(status_frame, textvariable=self.task_text, font=("Segoe UI", 11, "bold")).pack(side=tk.LEFT)
        ttk.Label(status_frame, textvariable=self.position_text, font=("Segoe UI", 11)).pack(side=tk.LEFT, padx=16)
        ttk.Label(status_frame, textvariable=self.status_text, font=("Segoe UI", 11)).pack(side=tk.RIGHT)

        self.canvas = tk.Canvas(self.root, width=CANVAS_SIZE, height=CANVAS_SIZE, bg="#f5f5f5", highlightthickness=1, highlightbackground="#cccccc")
        self.canvas.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))

    def _reset_canvas_state(self):
        self.canvas.delete("all")
        self.canvas.create_rectangle(0, 0, CANVAS_SIZE, CANVAS_SIZE, fill="#f8f8f8", outline="#eeeeee")
    ...
    def _redraw_canvas(self):
        ...
        for idx, target in enumerate(self.targets):
            ...
            canvas_x = int((x / 100.0) * (CANVAS_SIZE - 60) + 30)
            canvas_y = int((y / 100.0) * (CANVAS_SIZE - 60) + 30)
            is_completed = idx < self.completed_count
            color = "#90ee90" if is_completed else "#f0f0f0"
            outline = "#2e8b57" if is_completed else "#666666"
            self.canvas.create_oval(
                canvas_x - NODE_RADIUS,
                canvas_y - NODE_RADIUS,
                canvas_x + NODE_RADIUS,
                canvas_y + NODE_RADIUS,
                fill=color,
                outline=outline,
    ...
    def _handle_event(self, event):
        ...
        elif event_type in {"correct_click", "incorrect_click", "miss_click"}:
            self._move_cursor(event.get("x", 0), event.get("y", 0))
    ...
    def _move_cursor(self, x, y):
        ...
        if self.cursor_item is None:
            self.cursor_item = self.canvas.create_oval(0, 0, 0, 0, fill="red", outline="")
        self.canvas.coords(
            self.cursor_item,
            canvas_x - CURSOR_RADIUS,
            canvas_y - CURSOR_RADIUS,
            canvas_x + CURSOR_RADIUS,
            canvas_y + CURSOR_RADIUS,
        )
        self.canvas.itemconfigure(self.cursor_item, fill="red")

        if self.cursor_path_points:
            last_x, last_y = self.cursor_path_points[-1]
            self.canvas.create_line(last_x, last_y, canvas_x, canvas_y, fill="#ff4500", width=2)
    ...
    def _show_click(self, x, y, correct=True):
        ...
        outline = "green" if correct else "red"
        self.click_marker = self.canvas.create_oval(
            x - CLICK_MARKER_RADIUS,
            y - CLICK_MARKER_RADIUS,
            x + CLICK_MARKER_RADIUS,
            y + CLICK_MARKER_RADIUS,
            outline=outline,
            width=2,
